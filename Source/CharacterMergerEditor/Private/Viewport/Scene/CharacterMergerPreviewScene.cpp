#include "Viewport/Scene/CharacterMergerPreviewScene.h"
...
FCharacterMergerPreviewScene::FCharacterMergerPreviewScene(ConstructionValues CVS, float InFloorOffset)
	: FPreviewScene(CVS)
{
	DefaultSettings = UAssetViewerSettings::Get();
	check(DefaultSettings);

	RefreshDelegate = DefaultSettings->OnAssetViewerSettingsChanged().AddRaw(this, &FCharacterMergerPreviewScene::OnAssetViewerSettingsRefresh);
...
FCharacterMergerPreviewScene::~FCharacterMergerPreviewScene()
{
...
void FCharacterMergerPreviewScene::AddComponent(UActorComponent* Component, const FTransform& LocalToWorld,
	bool bAttachToRoot)
{
	FPreviewScene::AddComponent(Component, LocalToWorld, bAttachToRoot);

	USceneComponent* NewComponent = Cast<USceneComponent>(Component);
	if(bAttachToRoot && Root == nullptr) Root = NewComponent;
	else if(bAttachToRoot && Root != nullptr)
	{
		//NewComponent->AttachToComponent(Root, FAttachmentTransformRules::KeepRelativeTransform);
	}
}

void FCharacterMergerPreviewScene::RemoveComponent(UActorComponent* Component)
{
	if(Root == Component) return;
	FPreviewScene::RemoveComponent(Component);
}

void FCharacterMergerPreviewScene::UpdateScene(FPreviewSceneProfile& Profile, bool bUpdateSkyLight /*= true*/, bool bUpdateEnvironment  /*= true*/, bool bUpdatePostProcessing /*= true*/, bool bUpdateDirectionalLight /*= true*/)
{
	if (bUpdateSkyLight)
	{
...
FLinearColor FCharacterMergerPreviewScene::GetBackgroundColor() const
{
...
void FCharacterMergerPreviewScene::SetFloorOffset(const float InFloorOffset)
{
	FTransform FloorTransform(FRotator(0, 0, 0), FVector(0, 0, -(InFloorOffset)), FVector(4.0f, 4.0f, 1.0f));
...
void FCharacterMergerPreviewScene::SetProfileIndex(const int32 InProfileIndex)
{
...
void FCharacterMergerPreviewScene::Tick(float DeltaTime)
{
...
TStatId FCharacterMergerPreviewScene::GetStatId() const
{
	return TStatId();
}

const bool FCharacterMergerPreviewScene::HandleViewportInput(FViewport* InViewport, int32 ControllerId, FKey Key, float Delta, float DeltaTime, int32 NumSamples, bool bGamepad)
{
	bool bResult = false;
...
const bool FCharacterMergerPreviewScene::HandleInputKey(FViewport* InViewport, int32 ControllerId, FKey Key, EInputEvent Event, float AmountDepressed, bool Gamepad)
{
	if (Event == IE_Pressed)
	{
...
const UStaticMeshComponent* FCharacterMergerPreviewScene::GetFloorMeshComponent() const
{
	checkf(FloorMeshComponent != nullptr, TEXT("Invalid floor mesh component pointer"));
	return FloorMeshComponent;
}
void FCharacterMergerPreviewScene::SetFloorVisibility(const bool bVisible, const bool bDirect)
{
...
void FCharacterMergerPreviewScene::SetEnvironmentVisibility(const bool bVisible, const bool bDirect)
{
	// If not direct set visibility in profile and refresh the scene
	if (!bDirect)
	{
...
const float FCharacterMergerPreviewScene::GetSkyRotation() const
{
	checkf(DefaultSettings && DefaultSettings->Profiles.IsValidIndex(CurrentProfileIndex), TEXT("Invalid default settings pointer or current profile index"));
	return DefaultSettings->Profiles[CurrentProfileIndex].LightingRigRotation;
}
void FCharacterMergerPreviewScene::SetSkyRotation(const float SkyRotation)
{
	checkf(DefaultSettings && DefaultSettings->Profiles.IsValidIndex(CurrentProfileIndex), TEXT("Invalid default settings pointer or current profile index"));
...
const bool FCharacterMergerPreviewScene::IsUsingPostProcessing() const
{
	return bPostProcessing;
}
const int32 FCharacterMergerPreviewScene::GetCurrentProfileIndex() const
{
	return CurrentProfileIndex;
}

void FCharacterMergerPreviewScene::BindCommands()
{
	UICommandList = MakeShared<FUICommandList>();

	/*const FCharacterMergerPreviewSceneCommands& Commands = FCharacterMergerPreviewSceneCommands::Get();

	UICommandList->MapAction(Commands.ToggleFloor,
		FExecuteAction::CreateRaw(this, &FCharacterMergerPreviewScene::HandleToggleFloor));

	UICommandList->MapAction(Commands.ToggleEnvironment,
		FExecuteAction::CreateRaw(this, &FCharacterMergerPreviewScene::HandleToggleEnvironment));

	UICommandList->MapAction(Commands.TogglePostProcessing,
		FExecuteAction::CreateRaw(this, &FCharacterMergerPreviewScene::HandleTogglePostProcessing));*/
}

void FCharacterMergerPreviewScene::HandleToggleEnvironment()
{
	SetEnvironmentVisibility(!DefaultSettings->Profiles[CurrentProfileIndex].bShowEnvironment);
}
void FCharacterMergerPreviewScene::HandleToggleFloor()
{
	SetFloorVisibility(!DefaultSettings->Profiles[CurrentProfileIndex].bShowFloor);
}
void FCharacterMergerPreviewScene::HandleTogglePostProcessing()
{
	FPreviewSceneProfile& Profile = DefaultSettings->Profiles[CurrentProfileIndex];
...
void FCharacterMergerPreviewScene::OnAssetViewerSettingsRefresh(const FName& InPropertyName)
{
