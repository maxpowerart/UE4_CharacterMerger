...
FCMSkeletalMeshMerge::FCMSkeletalMeshMerge(USkeletalMesh* InMergeMesh, const TArray<USkeletalMesh*>& InSrcMeshList,
	const TArray<FCMSkelMeshMergeSectionMapping>& InForceSectionMapping, int32 InStripTopLODs,
	EMeshBufferAccess InMeshBufferAccess, FCMSkelMeshMergeUVTransforms* InSectionUVTransforms)
...
bool FCMSkeletalMeshMerge::DoMerge(TArray<FCMRefPoseOverride>* RefPoseOverrides)
{
	MergeSkeleton(RefPoseOverrides);
	const bool bResult = FinalizeMesh();

	return bResult;
}

void FCMSkeletalMeshMerge::MergeSkeleton(const TArray<FCMRefPoseOverride>* RefPoseOverrides)
{
...
bool FCMSkeletalMeshMerge::FinalizeMesh()
{
	bool Result = true;
	...
	for (int32 MeshIdx = 0; MeshIdx < SrcMeshList.Num(); MeshIdx++)
	{
		...
		if (SrcMesh)
		{
			...
			FCMMergeMeshInfo& MeshInfo = SrcMeshInfo[MeshIdx];
			...
			TArray<UMorphTarget*> MorphTargets = SrcMesh->GetMorphTargets();
			for(UMorphTarget* MTSource : MorphTargets)
			{
				MeshInfo.MorphMap.Add(MTSource->GetFName());

				/**TODO If failed, try to mapped vertex values*/
				for(auto Morph : MTSource->MorphLODModels[0].Vertices)
				{
					int32 VertexDelta = 0;
					for(int32 SectionIt = 0; SectionIt < MeshIdx; SectionIt++)
					{
						VertexDelta+= SrcMeshList[SectionIt]->GetImportedModel()->LODModels[0].NumVertices;
					}
					MeshInfo.MorphMap[MTSource->GetFName()].Add(Morph.SourceIdx + VertexDelta, Morph.PositionDelta);
				}
			}
		}
	}
...
void FCMSkeletalMeshMerge::MergeBoneMap(TArray<FBoneIndexType>& MergedBoneMap,
...
void FCMSkeletalMeshMerge::GenerateLODModel(int32 LODIdx)
{
	...
	for( int32 MergeIdx=0; MergeIdx < SrcMeshInfo.Num(); MergeIdx++ )
	{
		for(auto MorphMap : SrcMeshInfo[MergeIdx].MorphMap)
		{
			if(!MergedMorphMap.Contains(MorphMap.Key))
			{
				MergedMorphMap.Add(MorphMap.Key);
			}
			MergedMorphMap[MorphMap.Key].Append(MorphMap.Value);
		}
	}
	...
	for( int32 CreateIdx=0; CreateIdx < NewSectionArray.Num(); CreateIdx++ )
	{
		...
		for( int32 MergeIdx=0; MergeIdx < NewSectionInfo.MergeSections.Num(); MergeIdx++ )
		{
			...
			for( int32 VertIdx=MergeSectionInfo.Section->BaseVertexIndex; VertIdx < MaxVertIdx; VertIdx++ )
			{
				...
				CopyVertexFromSource<VertexDataType>(DestVert, SrcLODData, VertIdx, MergeSectionInfo, NumTotalVertices);
...
void FCMSkeletalMeshMerge::GenerateNewSectionArray(TArray<FCMNewSectionInfo>& NewSectionArray, int32 LODIdx)
{
...
bool FCMSkeletalMeshMerge::ProcessMergeMesh()
{
	bool Result=true;
...
int32 FCMSkeletalMeshMerge::CalculateLodCount(const TArray<USkeletalMesh*>& SourceMeshList) const
{
...
void FCMSkeletalMeshMerge::BuildReferenceSkeleton(const TArray<USkeletalMesh*>& SourceMeshList,
	FReferenceSkeleton& RefSkeleton, const USkeleton* SkeletonAsset)
{
	RefSkeleton.Empty();
...
void FCMSkeletalMeshMerge::OverrideReferenceSkeletonPose(const TArray<FCMRefPoseOverride>& PoseOverrides,
	FReferenceSkeleton& TargetSkeleton, const USkeleton* SkeletonAsset)
{
	for (int32 i = 0, PoseMax = PoseOverrides.Num(); i < PoseMax; ++i)
	{
		const FCMRefPoseOverride& PoseOverride = PoseOverrides[i];
		...
		for (int32 j = 0, BoneMax = PoseOverride.Overrides.Num(); j < BoneMax; ++j)
		{
			...
			{
				bool bOverrideBone = (PoseOverride.Overrides[j].OverrideMode == FCMRefPoseOverride::ChildrenOnly) ? false : true;
				...
				bool bOverrideChildren = (PoseOverride.Overrides[j].OverrideMode == FCMRefPoseOverride::BoneOnly) ? false : true;
...
bool FCMSkeletalMeshMerge::OverrideReferenceBonePose(int32 SourceBoneIndex, const FReferenceSkeleton& SourceSkeleton,
...
void FCMSkeletalMeshMerge::ReleaseResources(int32 Slack)
{
...
bool FCMSkeletalMeshMerge::AddSocket(const USkeletalMeshSocket* NewSocket, bool bIsSkeletonSocket)
{
...
void FCMSkeletalMeshMerge::AddSockets(const TArray<USkeletalMeshSocket*>& NewSockets, bool bAreSkeletonSockets)
{
...
void FCMSkeletalMeshMerge::BuildSockets(const TArray<USkeletalMesh*>& SourceMeshList)
{
...
void FCMSkeletalMeshMerge::OverrideSocket(const USkeletalMeshSocket* SourceSocket)
{
...
void FCMSkeletalMeshMerge::OverrideBoneSockets(const FName& BoneName,
	const TArray<USkeletalMeshSocket*>& SourceSocketList)
{
...
void FCMSkeletalMeshMerge::OverrideMergedSockets(const TArray<FCMRefPoseOverride>& PoseOverrides)
{
	for (int32 i = 0, PoseMax = PoseOverrides.Num(); i < PoseMax; ++i)
	{
		const FCMRefPoseOverride& PoseOverride = PoseOverrides[i];
...
template <typename VertexDataType>
void FCMSkeletalMeshMerge::CopyVertexFromSource(VertexDataType& DestVert, const FSkeletalMeshLODRenderData& SrcLODData,
	int32 SourceVertIdx, const FCMMergeSectionInfo& MergeSectionInfo, int32 NumVertexes)
{
	/**Find vertex right index */
	FVector Offset = FVector::ZeroVector;
	for(auto Morph : MergedMorphMap[FName("Size")])
	{
		if(SourceVertIdx + NumVertexes == Morph.Key)
		{
			Offset = Morph.Value;
			break;
		}
